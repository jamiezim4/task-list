...
generatorHandler({
    ...
    onGenerate: async (options) => {
        ...
        const prismaLocation = prismaOutputLocation.startsWith("@") || extensions.some((ext) => prismaOutputLocation.endsWith(ext)) ? prismaOutputLocation : prismaOutputLocation.startsWith("./") ? `./${posix.join(prismaOutputLocation, "index.js")}` : posix.join(prismaOutputLocation, "index.js");
        if (!prismaLocation) {
        ...
        if (outputLocation === defaultOutput) {
            await generateOutput(options.dmmf, prismaTypes, prismaLocation.startsWith("@") ? prismaLocation : posix.join(prismaLocation, "index.js"), join(outputLocation, "../esm/generated.d.ts"), config, "esm");
        }
    }
});
...
async function generateOutput(dmmf, prismaTypes, prismaLocation, outputLocation, config, datamodel) {
    ...
    const nodes = ts.factory.createNodeArray(config.generateDatamodel === "true" ? [
