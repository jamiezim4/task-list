...
(0, _generatorhelper.generatorHandler)({
    ...
    onGenerate: async (options)=>{
        ...
        const prismaLocation = prismaOutputLocation.startsWith('@') || extensions.some((ext)=>prismaOutputLocation.endsWith(ext)) ? prismaOutputLocation : prismaOutputLocation.startsWith('./') ? `./${_nodepath.posix.join(prismaOutputLocation, 'index.js')}` : _nodepath.posix.join(prismaOutputLocation, 'index.js');
        if (!prismaLocation) {
        ...
        if (outputLocation === defaultOutput) {
            await generateOutput(options.dmmf, prismaTypes, prismaLocation.startsWith('@') ? prismaLocation : _nodepath.posix.join(prismaLocation, 'index.js'), (0, _nodepath.join)(outputLocation, '../esm/generated.d.ts'), config, 'esm');
        }
    }
});
...
async function generateOutput(dmmf, prismaTypes, prismaLocation, outputLocation, config, datamodel) {
    ...
    const nodes = _typescript.default.factory.createNodeArray(config.generateDatamodel === 'true' ? [
